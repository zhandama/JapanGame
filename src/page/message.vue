      ...
      <van-pull-refresh v-model="refreshing" @refresh="onRefresh">
        <van-list v-model="loading" :finished="finished" @load="onLoad">
          <ul class="pl-25" v-if="list.length > 0">
            ...
            <li class="border-top py-25 pr-25"  v-for="(item, index) in filterList" :key="index">
              <router-link :to="{name:'Chat',query:{goodsId:item.goodsId,otherId:item.otherId}}">
